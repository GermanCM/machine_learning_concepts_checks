...
mask_target_not_2 = [(target_values==0)|(target_values==1)]
#%%
features_values_2_D = ds.data[:, 0:2][tuple(mask_target_not_2)]

#%%
target_values = ds.target[tuple(mask_target_not_2)]
...
ada = AdalineSGD(n_iter=30, eta=0.01, random_state=1)
ada_model_whole_data = ada.fit(X_std, y)
...
plt.title('Adaline trained with all of the data - Stochastic Gradient Descent')
plt.xlabel('sepal length [standardized]')
plt.ylabel('petal length [standardized]')
plt.legend(loc='upper left')
plt.show()
